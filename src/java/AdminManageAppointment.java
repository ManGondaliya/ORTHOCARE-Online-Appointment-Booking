...
import java.sql.*;
 ...
 @WebServlet("/AdminManageAppointment")
public class AdminManageAppointment extends HttpServlet {
       ...
       try
       {
           ...
           int rowsUpdated = pstmt.executeUpdate();
            ...
            if (rowsUpdated > 0) {
                ...
                response.getWriter().println("window.location='manageAppointment.jsp';"); 
                response.getWriter().println("</script>");
            } else {
                response.setContentType("text/html;charset=UTF-8");
                response.getWriter().println("<script type='text/javascript'>");
                response.getWriter().println("alert('Failed to update appointment. Please try again.');");
                response.getWriter().println("window.location='adminManageAppointments.jsp';");
                response.getWriter().println("</script>");
            }
